...
class GetDataThread(QtCore.QThread):
    def __init__(self, main_window, parent=None):
        ...
        self.start_freq_set_ok_flag = False
        self.stop_freq_set_ok_flag = False
        self.start_freq_send_once = False
        self.stop_freq_send_once = False
    ...
    def run(self):
        while True:
            ...
            if self.main_window.freq_range_changing_flag is True:

                start_freq_command = 'SA01' + str(self.main_window.start_freq) + '.'

                if self.start_freq_set_ok_flag is False:
                    if self.start_freq_send_once is False:
                        self.main_window.serial_port.write(start_freq_command)
                        print('sending ' + start_freq_command)
                        self.start_freq_send_once = True
                    if not line.startswith('set_freq_start:'):
                        continue
                    elif line[15:21] == str(self.main_window.start_freq):
                        self.start_freq_set_ok_flag = True
                        print('start freq set ok')
                        continue
                    else:
                        self.start_freq_send_once = False
                        print('set fail, send again @_@')
                        continue

                stop_freq_command = 'SA02' + str(self.main_window.stop_freq) + '.'

                if self.stop_freq_set_ok_flag is False:
                    if self.stop_freq_send_once is False:
                        self.main_window.serial_port.write(stop_freq_command)
                        print('sending ' + stop_freq_command)
                        self.stop_freq_send_once = True
                    if not line.startswith('set_freq_stop:'):
                        continue
                    elif line[14:20] == str(self.main_window.stop_freq):
                        self.stop_freq_set_ok_flag = True
                        print('stop freq set ok')
                        continue
                    else:
                        self.stop_freq_send_once = False
                        print('set fail, send again @_@')
                ...
                if self.start_freq_set_ok_flag is True and self.stop_freq_set_ok_flag is True:
                    self.main_window.freq_range_changing_flag = False
                    self.start_freq_set_ok_flag = False
                    self.stop_freq_set_ok_flag = False
                    self.start_freq_send_once = False
                    self.stop_freq_send_once = False
                    self.main_window.spectrum_data = OrderedDict()
                    print('set freq range OK!')
                    continue
                # time.sleep(0.01)
                # time.sleep(0.01)
            ...
            if self.main_window.show_mode == 'spectrum':
                ...
                if line_list[0] != '-':
                    ...
                    continue
                try:
                    freq_spectrum = int(line_list[1]) #/ 1000
