...
class MainWindow(QtGui.QMainWindow):
    ...
    def __init__(self, force_port_num=None, parent=None):
        ...
        self.setWindowTitle('Fine Spectrum Analyzer')
        ...
        self.serialPortChangeButton = QtGui.QPushButton('Change\nSerial Port')
        self.serialPortChangeButton.setFont(QtGui.QFont("Calibri", 10))
        ...
        self.fixScreenButton = QtGui.QPushButton('Fix Screen')
        self.fixScreenButton.setFont(QtGui.QFont("Calibri", 12))
        self.fixScreenButton.setStyleSheet('''color:blue''')
        self.fixScreenButton.setFixedWidth(100)
        ...
        frame1_grid.addWidget(self.setFreqRangeButton, 2, 2, 2, 1, QtCore.Qt.AlignLeft)
        frame1_grid.addWidget(self.fixScreenButton, 2, 3, 2, 1, QtCore.Qt.AlignRight)
        ...
        self.start_freq = 889000
        self.stop_freq = 891000
        self.freq_range_changing_flag = False
    ...
    @QtCore.pyqtSlot()
    def fix_screen(self):
        keys = self.spectrum_data.keys()
        min_freq = min(keys)
        max_freq = max(keys)
        print(min_freq)
        print(max_freq)
        self.show_canvas.start_freq = min_freq
        self.show_canvas.stop_freq = max_freq

        data = dict(self.spectrum_data)
        self.spectrum_data = OrderedDict(sorted(data.items(), key=lambda t: t[0]))
    ...
    def start_stop_freq_show_change(self):
        if len(self.startFreqShowEdit.text()) != 6:
            print('wrong frequency length, please enter like \'XXXXXX\'')
        if len(self.stopFreqShowEdit.text()) != 6:
            print('wrong frequency length, please enter like \'XXXXXX\'')

        try:
            start_freq_show_pre = int(self.startFreqShowEdit.text())
        except Exception as e:
            print(e)
            return
        try:
            stop_freq_show_pre = int(self.stopFreqShowEdit.text())
        except Exception as e:
            print(e)
            return

        if start_freq_show_pre < 850000 \
                or start_freq_show_pre > stop_freq_show_pre \
                or stop_freq_show_pre > 928000:
            print('wrong frequency range')
            return

        self.show_canvas.start_freq = start_freq_show_pre
        self.start_freq = start_freq_show_pre
        self.show_canvas.stop_freq = stop_freq_show_pre
        self.stop_freq = stop_freq_show_pre
        self.freq_range_changing_flag = True
    ...
    #     self.startFreqShowEdit.setDisabled(True)
    #     self.stopFreqShowEdit.setDisabled(True)
    #     self.fixScreenButton.setDisabled(True)
    ...
    #     self.fixScreenButton.setDisabled(False)
    #     self.timeScaleEdit.setDisabled(True)
    #     self.clear_mark()
    ...
    def show_click_position(self):
        if self.show_mode == 'spectrum':
            if self.show_canvas.mark_number == 1:
                self.mark1Label.setText((':    %.0f KHz' % self.show_canvas.mark1_x) +
                                        (':    %.2f dbm' % self.show_canvas.mark1_y))
            elif self.show_canvas.mark_number == 2:
                self.mark2Label.setText((':    %.0f KHz' % self.show_canvas.mark2_x) +
            ...
            if self.show_canvas.mark1_x is not None and \
                ...
                self.deltaContentLabel.setText(
                    (':     %.0f KHz' % (self.show_canvas.mark2_x - self.show_canvas.mark1_x)) +
